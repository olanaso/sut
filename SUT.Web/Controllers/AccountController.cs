...
namespace Sut.Web.Controllers
{
    ...
    {
        private readonly ILogService<AccountController> _log;
        ...
        {
            _log = new LogService<AccountController>();
        ...
        public ActionResult Login(string returnUrl)
        {
            if (returnUrl != null)
            {
                ...
                if (returnUrl.Contains("Account/resolver"))
                {
                    return resolver(returnUrl);
        ...
        {
            string returnUrl = "http://192.168.120.205:9091/";

            int post=t.IndexOf("=");
            t= t.Substring(post+1);
            ...
            model.UserName =dataRespuesta.d.documento;
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            else
            {
                ...
                if (!ModelState.IsValid && model.NombreCompleto != "omitir")
                {
                    ViewBag.TipoDoc = listtipodoc.Select(x => new SelectListItem()
                    {
                        Value = x.MetaDatoId.ToString(),
            ...
            if (user != null)
            {
                ...
                //List<RolMenu> listarolmenu = _rolMenuService.GetByRolMenu((short)user.Rol);
